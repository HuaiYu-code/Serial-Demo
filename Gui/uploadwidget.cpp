    ...
    ui(new Ui::UploadWidget)
{
	...
	connect(&PUB_SS, SIGNAL(signalcloseUpload()), this, SLOT(slot_closeUpload()));
...
void UploadWidget::SoltCurrentPkgNum(int currentCounts)
{
	...
	qDebug() << "SET UPLOAD CURRENT:"<< currentCounts;
...
#if  HHT_CHINA
        hint->setMassage(QString::fromUtf8("应用上传失败。  "),-2);//鸿合助手向智能设备发送数据失败
#else
...
void UploadWidget::initValue()
{
    ...
    connect(closeTimer2,SIGNAL(timeout()),this,SLOT(closeTimer2Event()), Qt::QueuedConnection);//定时关闭
...
void UploadWidget::drawInnerCircle(QPainter *painter)
{
    ...
    coverGradient.setColorAt(1.0,QColor("#2a2929"));
...
//    graphGradient.setColorAt(1.0,QColor(255,0,128));
    graphGradient.setColorAt(1.0, QColor("#38a6df"));
