...
//	qDebug() << "log UserDataPath: " << UserDataPath;
	return UserDataPath;
}
/***********************************************************
 ...
 /*   if (0 == strlen(g_acCurAppRunDir))
    {
		...
		vos_snprintf(g_acCurAppRunDir, sizeof(g_acCurAppRunDir), "%s/log", strAppPath.toLocal8Bit().data());
    }*/
		QString strAppPath;
		strAppPath = Pub_GetLogDir();
		if (dir.exists(strAppPath)== false)
		{
			dir.mkdir(strAppPath);
		}
        vos_snprintf(acLogPath, sizeof(acLogPath), "%s/hht_log.txt", strAppPath.toLocal8Bit().data());
